...
export async function submitRsvp(values, config) {
  const portalId = (config.portalId || "").trim();
  const formId = (config.formId || "").trim();
  let endpoint = config.endpoint || "";
  let payload = { ...values, status: "Confirmed", source: "sbc-summit-2026-rsvp" };
  ...
  if (portalId || formId) {
    ...
    const mapping = {
      ...
      company: "company", jobTitle: "jobtitle", days: "sbc_rsvp_days",
      status: "sbc_rsvp_status", source: "sbc_submission_source",
    };
    payload = {
  ...
  if (!endpoint) throw new Error("RSVP submissions are not configured.");
  ...
  try {
    ...
    if (!response.ok) throw new Error(`RSVP submission failed (${response.status}).`);
  } finally {
